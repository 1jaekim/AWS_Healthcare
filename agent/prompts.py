...
판정 기준:
- OK: 제공된 근거가 해당 기준 조건을 충족한다고 지지한다.
- NOT_OK: 제공된 근거가 해당 기준 조건과 충돌한다.
- UNKNOWN: 근거가 없거나, 서로 충돌하거나, 시점·단위·문맥이 부족하다.
- OK 또는 NOT_OK에는 반드시 입력에 실제 존재하는 source_id가 하나 이상 필요하다.
- source_id가 없으면 반드시 UNKNOWN이다.

기록에 언급이 없을 때의 판단:
임상 기록은 있는 것을 적는다. 그래서 "적혀 있지 않다"와 "확인할 수 없다"는 다르다.
- 병력·진단·약물처럼 해당하면 기록에 남는 항목이고, 그 계통이나 관련 병력을
  서술한 내용이 근거에 있는데 그 항목만 언급되지 않았다면 없는 것으로 보고
  판정한다. 인용 출처는 그 서술이다.
  예: 심혈관 병력과 현재 상태를 서술한 기록에 심근경색 언급이 없다 -> 없음으로 본다.
- 그 계통을 전혀 다루지 않았거나 근거 자체가 비어 있으면 UNKNOWN이다.
- 수치 임계값이 있는 기준은 측정값이 필요하다. 증상이 없다는 서술만으로
  측정값을 대신하지 않는다.
  예: "간 수치가 정상 상한의 3배 초과" 기준에 검사 결과가 없다 -> UNKNOWN.
- 기간 조건이 있는 기준은 날짜가 필요하다. 날짜가 없거나 기간을 벗어난 관찰로
  판정하지 않는다.

기준이 대상자에게 적용되지 않을 때:
성별·연령 때문에 기준이 성립하지 않으면 UNKNOWN으로 두고 적용 불가임을
rationale에 적는다. 해당 없음을 충족으로 바꾸어 OK로 답하지 않는다.
예: 남성 대상자에게 임신 관련 기준, 소아에게 성인 한정 기준.
