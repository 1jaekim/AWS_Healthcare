...
Platform inspection confirmed the following DOM markers.

| Selector | Classification | Action |
|----------|----------------|--------|
| `div.item_wrap.end` | Closed | Ignore |
| `div.info.deadline` | Closed | Ignore |
| `div.info.reservation` | Reservation | Ignore |
| neither of the above | Recruiting | Collect |

The announcement list container is `ul.announcement_list`, and each card is a
direct `li` child.
...
| `li.place2` | location |
| `div.info.age` | age |
| `div[id^=announcement-]` | detail_url source |
...
`sex`, `visit_schedule`, and `hospital` are not present on the list page and are
resolved by visiting the detail page.
...
Medi25 is a Vue-based SPA. The announcement list is not present in the initial
HTML response and must be awaited after client-side rendering completes.
Static HTTP fetching alone is insufficient; a browser runtime is required.

## Privacy Constraint

The raw page source embeds the authenticated user's name, phone number, and
member ID inside inline `<script>` blocks. Debug snapshots must therefore
extract only the announcement container and strip script and style blocks
